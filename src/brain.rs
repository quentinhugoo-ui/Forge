...
pub const BRAIN_MAPS_COMMAND_DESCRIPTION: &str = "Open contained WebExplorer on Google Earth for map/globe exploration, explicit coordinates, or geospatial inspection. Do not read device location silently; current-location use requires explicit user permission.";
...
pub const BRAIN_AIRBNB_COMMAND_DESCRIPTION: &str = "Use Airbnb for Airbnb mentions and vacation/trip lodging intent with destination, dates, month, guests, budget, home/apartment rental, or accommodation search. Prefer over /googleweb_ even when the user says only they want to travel somewhere.";
...
pub fn brain_maps_codeact_template() -> BrainGeneralCodeActTemplate {
    let mut template = BrainGeneralCodeActTemplate {
        ...
        section: "webexplorer".to_string(),
        purpose: "Open the contained native WebExplorer on Google Earth while the assistant keeps conversing in the left transcript. Default opens the fixed Earth view requested by the user; optional latitude and longitude slots let the LLM target explicit WGS84 coordinates. Device location must never be read silently; current-location flows require explicit user permission before any coordinate is used.".to_string(),
        ...
        slots: vec![
            BrainCodeActTemplateSlot {
                ...
                allowed_values: Vec::new(),
                description: "Human-readable place or coordinate target preserved in the proof. Leave default_google_earth_view when the user only asks to open Maps/Earth.".to_string(),
            },
            BrainCodeActTemplateSlot {
...
pub fn brain_airbnb_codeact_template() -> BrainGeneralCodeActTemplate {
    let mut template = BrainGeneralCodeActTemplate {
        ...
        section: "webexplorer".to_string(),
        purpose: "Open the contained split-screen WebExplorer on Airbnb.com. Prefer this command over /googleweb_ whenever the user mentions Airbnb or expresses a vacation/trip intent with a destination and period, including broad requests like 'je veux partir en vacances au Japon en septembre' even when the user does not explicitly say lodging/rental. Also prefer it for trip lodging, house/apartment/home rental, destination dates, guests, budget, islands/cities or similar travel accommodation intent. The LLM must write its own natural user-facing sentence adapted to the user's request before activating the command, and must also copy that same LLM-authored sentence into the say slot so command-only render paths still show a normal answer. The host must not synthesize that sentence. The LLM then activates this CodeAct, and the application renders the action event automatically.".to_string(),
