...
pub const BRAIN_RENAME_SESSION_COMMAND_DESCRIPTION: &str = "Rename the current chat session with a short, relevant LLM-authored title after identifying the first user message subject. The event is internal and must not be described in visible prose. Use 2-5 natural words, like Codex or Claude; avoid copying the prompt or using only a proper noun.";
...
pub fn brain_rename_session_codeact_template() -> BrainGeneralCodeActTemplate {
    let mut template = BrainGeneralCodeActTemplate {
        ...
        section: "general".to_string(),
        purpose: "Rename the current session from the LLM's short, product-quality title for the first user message topic, keeping sidebar recents and Brain archive history aligned without visible rename chatter.".to_string(),
        ...
        slots: vec![
            BrainCodeActTemplateSlot {
                ...
                default_value: String::new(),
                allowed_values: Vec::new(),
                description: "Short relevant session title, 2-5 natural words, nominal and specific like Codex or Claude. Do not copy the prompt and do not use only a proper noun.".to_string(),
            },
            BrainCodeActTemplateSlot {
