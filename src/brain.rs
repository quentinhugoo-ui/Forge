...
pub fn brain_general_codeact_templates() -> Vec<BrainGeneralCodeActTemplate> {
    vec![
        ...
        brain_questionnaire_codeact_template(),
        brain_plan_codeact_template(),
        brain_local_actions_codeact_template(),
...
fn brain_local_action_atlas_codeact_template(
    command: &str,
    purpose: &str,
    result_schema: &str,
) -> BrainGeneralCodeActTemplate {
...
pub fn brain_local_actions_codeact_template() -> BrainGeneralCodeActTemplate {
    brain_local_action_atlas_codeact_template(
        BRAIN_LOCAL_ACTIONS_COMMAND,
        "Call the local AgentAction JSON atlas before choosing a Windows/local action route. Use this in General, Science, Coding or any specialized Brain whenever the task may involve the computer, files, apps, browser, documents, code, Git/GitHub, cloud CLIs, virtualization, Windows settings, packages or automation and the exact AGENT_ACTION_JSON action is not obvious. The host executes this CodeAct as read-only action:\"capabilities\" and returns LOCAL_ACTIONS_RESULT for the next loop-stream step.",
...
pub fn brain_gmail_codeact_template() -> BrainGeneralCodeActTemplate {
    let mut template = BrainGeneralCodeActTemplate {
        ...
        section: "webexplorer".to_string(),
        purpose: "Strict two-phase Gmail CodeAct for mailbox tasks. Use /gmail_ whenever the user asks about their mailbox, emails, Gmail messages, attachments, unread mail, senders, subjects, invoices, confirmations, appointments, or asks to draft/reply to an email. First emit bare /gmail_ to receive GMAIL_TEMPLATE_RESULT. Then fill the returned compact template with template_proof_hash, intent, query/keywords/message_id or draft fields. Default mode=gmail_api executes the bounded Gmail REST bridge and returns GMAIL_RESULT; mode=split_webexplorer opens the contained Gmail surface only for visual access and must not be used when the user expects a mailbox report. No email is sent automatically. Gmail API execution keeps the narrowest official scope possible: metadata for metadata-only lists, readonly for search/read, compose for drafts, send only after explicit user approval. The LLM writes its own short natural user-facing sentence, for example Je verifie les mails recents. Do not mention templates, CodeAct, API bridge, dedicated access, authorization, or result lists before GMAIL_RESULT; mention authorization only when the Gmail result says auth_required. The host must not synthesize that sentence.".to_string(),
...
pub fn brain_questionnaire_codeact_template() -> BrainGeneralCodeActTemplate {
    let mut template = BrainGeneralCodeActTemplate {
        ...
        purpose: "Ask a short paginated questionnaire when the user's goal is clear but key choices, constraints, scope, files, risks or success criteria are missing. Use only if guessing would hurt the result. Max 5 questions, one per page, three concrete expert options each; host adds Other. Skip trivial tasks or obvious safe defaults.".to_string(),
        ...
        slots: vec![
            BrainCodeActTemplateSlot {
                ...
                allowed_values: Vec::new(),
                description: "Short questionnaire title.".to_string(),
            },
            BrainCodeActTemplateSlot {
                ...
                default_value: String::new(),
                allowed_values: Vec::new(),
                description: "One or two concise sentences explaining why these answers matter.".to_string(),
            },
            BrainCodeActTemplateSlot {
                name: "q1".to_string(),
                required: true,
                default_value: String::new(),
                allowed_values: Vec::new(),
                description: "Page 1 question. Put choices in q1_options.".to_string(),
            },
            BrainCodeActTemplateSlot {
                ...
                default_value: String::new(),
                allowed_values: Vec::new(),
                description: "Exactly three concrete expert options for q1, separated by |. Format: `Label (Tag) - benefit, tradeoff, when to choose`. Use `(Recommended)` when useful. Color labels may include `color:#hex` or `colors:#hex,#hex`. No Option 1/2/3, vague answers or Other.".to_string(),
            },
            BrainCodeActTemplateSlot {
                name: "q2".to_string(),
                required: false,
                default_value: String::new(),
                allowed_values: Vec::new(),
                description: "Page 2 question when needed. Put choices in q2_options.".to_string(),
            },
            BrainCodeActTemplateSlot {
                ...
                default_value: String::new(),
                allowed_values: Vec::new(),
                description: "Exactly three concrete expert options for q2, separated by |. Same format as q1_options; no vague answers or Other.".to_string(),
            },
            BrainCodeActTemplateSlot {
                name: "q3".to_string(),
                required: false,
                default_value: String::new(),
                allowed_values: Vec::new(),
                description: "Page 3 question when needed. Put choices in q3_options.".to_string(),
            },
            BrainCodeActTemplateSlot {
                ...
                default_value: String::new(),
                allowed_values: Vec::new(),
                description: "Exactly three concrete expert options for q3, separated by |. Include an ambitious/quality option when useful; no Other.".to_string(),
            },
            BrainCodeActTemplateSlot {
                name: "q4".to_string(),
                required: false,
                default_value: String::new(),
                allowed_values: Vec::new(),
                description: "Page 4 question only when useful. Put choices in q4_options.".to_string(),
            },
            BrainCodeActTemplateSlot {
                ...
                default_value: String::new(),
                allowed_values: Vec::new(),
                description: "Exactly three concrete expert options for q4, separated by |. No vague answers or Other.".to_string(),
            },
            BrainCodeActTemplateSlot {
                name: "q5".to_string(),
                required: false,
                default_value: String::new(),
                allowed_values: Vec::new(),
                description: "Page 5 final question only when necessary. Put choices in q5_options.".to_string(),
            },
            BrainCodeActTemplateSlot {
                ...
                default_value: String::new(),
                allowed_values: Vec::new(),
                description: "Exactly three concrete expert options for q5, separated by |. No vague answers or Other.".to_string(),
            },
            BrainCodeActTemplateSlot {
                name: "mode".to_string(),
                required: false,
                ...
                description: "Render one question per page; host adds Other.".to_string(),
            },
            BrainCodeActTemplateSlot {
                name: "output".to_string(),
                required: false,
                ...
                description: "Render above composer and hide CodeAct metadata.".to_string(),
            },
        ],
    };
    template.proof_hash = Hash::for_blob(canonical_brain_general_codeact_template(&template).as_bytes()).as_hex();
...
pub fn brain_science_codeact_template() -> BrainGeneralCodeActTemplate {
    let mut template = BrainGeneralCodeActTemplate {
        ...
        purpose: "Activate /sciencebrain_ when the LLM understands that the user's task belongs to the Science/Engineering/3D Brain: science, engineering, mathematics, biology, chemistry, physics, cryptography, optimization, formal analysis, electronics, mechanics, robotics, CAD/3D engineering, physical product/prototype engineering, Banger, Monster, /newcompute_ or future Banger 3D work. Do not activate only for looking at images of artworks, sculptures, ceramics, forms, style, composition or aesthetics unless the user asks for material science, structure, manufacturing, CAD, simulation, measurements or engineering constraints. The host injects only this segment catalog after the LLM has activated the command; the app must not perform semantic routing before the LLM.".to_string(),
        ...
        slots: vec![
            ...
            BrainCodeActTemplateSlot {
                ...
                default_value: String::new(),
                allowed_values: Vec::new(),
                description: "Short LLM-authored reason for activating /sciencebrain_, strictly grounded in the user's words. Do not invent a discipline, goal, material or engineering intent that the user did not request.".to_string(),
            },
            BrainCodeActTemplateSlot {
                name: "output".to_string(),
                required: false,
    ...
    let mut template = BrainGeneralCodeActTemplate {
        ...
        slots: vec![
            ...
            BrainCodeActTemplateSlot {
                ...
                required: false,
                default_value: BRAIN_LOCAL_ACTIONS_COMMAND.to_string(),
                allowed_values: Vec::new(),
                description: "Optional additional CodeAct drafts or command names that should start inside this specialized Brain, separated by |. The host always seeds /local_actions_ by default for local AgentAction JSON atlas discovery.".to_string(),
            },
            BrainCodeActTemplateSlot {
...
mod tests {
    ...
    fn brain_exposes_gmail_as_general_codeact_template() {
        ...
        assert!(BRAIN_GMAIL_COMMAND_DESCRIPTION.contains("mailbox, emails, Gmail messages"));
        assert!(BRAIN_CODEACT_ROUTING_RULES.contains("Gmail access/authorization intent"));
        assert!(BRAIN_CODEACT_ROUTING_RULES.contains("consulter mes mails"));
        assert!(template.purpose.contains("mailbox, emails, Gmail messages"));
        assert!(template.slots.iter().any(|slot| {
            slot.name == "intent"
                && slot.required
    ...
    #[test]
    fn brain_exposes_questionnaire_as_general_codeact_template() {
        ...
        assert_eq!(template.proof_hash.len(), 40);
        assert!(template.purpose.contains("goal is clear"));
        assert!(template.purpose.contains("Skip trivial tasks"));
        assert!(template.slots.iter().any(|slot| slot.name == "q1" && slot.required));
        assert!(template.slots.iter().any(|slot| {
            slot.name == "q1_options"
                && slot.description.contains("color:#hex")
                && slot.description.contains("colors:#hex,#hex")
        }));
        assert!(template.slots.iter().any(|slot| {
