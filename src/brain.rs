...
pub const BRAIN_RENAME_SESSION_COMMAND_DESCRIPTION: &str = "Rename the current chat session with exactly one internal prefix tag /rename_session_<short title>_ at the start of the first assistant response. The app strips the tag before display and uses the title everywhere the session name appears, but it does not reason about or repair the title. The event is internal: never mention, explain, echo, format or discuss the rename in visible prose. Use 2-5 precise natural words, preserve exact canonical proper names and work titles, and avoid lazy keyword bags or damaged titles.";
...
pub fn brain_rename_session_codeact_template() -> BrainGeneralCodeActTemplate {
    let mut template = BrainGeneralCodeActTemplate {
        ...
        section: "general".to_string(),
        purpose: "Rename the current session from the LLM's short, product-quality title for the first user message topic. Preferred activation is an internal prefix tag /rename_session_<short title>_ before visible prose; the app strips the tag, uses the title everywhere the session name appears, and keeps Brain archive history aligned without visible rename chatter. The app does not reason about or repair the title; the LLM must choose it well.".to_string(),
        ...
        slots: vec![
            BrainCodeActTemplateSlot {
                ...
                default_value: String::new(),
                allowed_values: Vec::new(),
                description: "Short relevant session title used in /rename_session_<short title>_: 2-5 natural words, nominal and specific like Codex or Claude. Preserve exact canonical proper names and work titles from the user request; do not use lazy keyword bags, damaged names, a full prompt copy, or only a proper noun.".to_string(),
            },
            BrainCodeActTemplateSlot {
