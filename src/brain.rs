...
pub const BRAIN_MAPS_COMMAND_DESCRIPTION: &str = "Open contained WebExplorer on Google Earth when a geographic place is detected without travel/vacation/stay intent: cities, places, countries, regions, local weather at a place, where-is questions, maps, routes, coordinates, globe exploration, or geospatial inspection. Prefer over /sciencebrain_, /airbnb_, and /googleweb_ for ordinary geographic context; use Brain home city when no target is specified. Do not read device location silently; current-location use requires explicit user permission.";
...
pub fn brain_maps_codeact_template() -> BrainGeneralCodeActTemplate {
    let mut template = BrainGeneralCodeActTemplate {
        ...
        section: "webexplorer".to_string(),
        purpose: "Open the contained native WebExplorer on Google Earth while the assistant keeps conversing in the left transcript. Use this when a geographic place is detected without travel/vacation/stay intent: cities, places, countries, regions, local weather at a place, where-is questions, maps, routes, coordinates, Google Earth, or ordinary geographic context. Prefer it over /sciencebrain_, /airbnb_, and /googleweb_ for geography alone; use /airbnb_ instead when the same place appears with voyage, vacances, partir, visiter, tourisme, sejour, destination, dates, guests, lodging, accommodation, booking, or stay-search intent. Default uses the Brain home city when no target is specified; optional latitude and longitude slots let the LLM target explicit WGS84 coordinates. Device location must never be read silently; current-location flows require explicit user permission before any coordinate is used.".to_string(),
...
pub fn brain_airbnb_codeact_template() -> BrainGeneralCodeActTemplate {
    let mut template = BrainGeneralCodeActTemplate {
        ...
        section: "webexplorer".to_string(),
        purpose: "Open the contained split-screen WebExplorer on Airbnb.com. Use this command when a geographic place is detected together with travel/vacation/stay language: voyage, vacances, partir, visiter, tourisme, sejour, destination, dates, guests, lodging, accommodation, hotel-like stay, home/apartment rental, vacation rental, booking, budget for stays, or short-term stay search. Do not use it for city facts, local weather, geography, maps, routes, or ordinary geographic context without travel/vacation/stay intent; those belong to /maps_. The LLM must write its own natural user-facing sentence adapted to the user's request before activating the command, and must also copy that same LLM-authored sentence into the say slot so command-only render paths still show a normal answer. The host must not synthesize that sentence. The LLM then activates this CodeAct, and the application renders the action event automatically.".to_string(),
