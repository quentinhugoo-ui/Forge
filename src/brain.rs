...
pub const BRAIN_CODEACT_ROUTING_RULES: &str = "Brain segment priority: while the active segment is general, the LLM must classify the user's task by domain before answering. If the task belongs to science, engineering, mathematics, biology, chemistry, physics, cryptography, optimization, formal analysis, physical product design, electronics, mechanics, robotics, CAD/3D, Banger, Monster, /newcompute_ or future Banger 3D work, first activate /sciencebrain_ before giving the specialized answer. Physical product or prototype conception is engineering by default, even when the object is ordinary or newly mentioned. Ordinary city, place, local weather, map, route, country, region, coordinates, Google Earth, where-is, or geographic-context requests are geospatial tasks: prefer /maps_ and do not activate /sciencebrain_ unless the user explicitly asks for scientific meteorology, climate modeling, physics, engineering, or compute analysis. This is semantic domain routing, not keyword routing: the LLM must infer the implied domain from the user's natural-language request, even when the object, field or project name has never appeared in this Brain. If the task belongs to software engineering, coding, websites, applications, repositories, debugging, refactoring, tests, architecture, scripts, Rust, TypeScript, Electron, APIs, build systems or developer tooling, first activate /codingbrain_ before giving the specialized answer. Module priority: when the user asks about a city, place, country, region, island, local weather for a place, route, map, coordinates, Google Earth, where something is, or any geographic context, prefer /maps_ over /sciencebrain_, /airbnb_, and /googleweb_; use the Brain home city as the default maps target when no place is specified. Use /airbnb_ only for explicit Airbnb, lodging, accommodation, hotel-like stay, vacation rental, house/apartment/home rental, guests, booking, budget for stays, or short-term stay intent. Do not use /airbnb_ for city facts, weather, geography, maps, or generic destination discussion without lodging/accommodation intent. /googleweb_ is only for generic web research that is not owned by a more specific module. Clarifying questionnaire tools belong to specialized Brain catalogs; while the active segment is general, switch to the correct Brain segment before opening a questionnaire for specialized work.";
...
pub const BRAIN_MAPS_COMMAND_DESCRIPTION: &str = "Open contained WebExplorer on Google Earth for cities, places, countries, regions, local weather at a place, where-is questions, maps, routes, coordinates, globe exploration, or geospatial inspection. Prefer over /sciencebrain_, /airbnb_, and /googleweb_ for ordinary geographic context; use Brain home city when no target is specified. Do not read device location silently; current-location use requires explicit user permission.";
...
pub const BRAIN_AIRBNB_COMMAND_DESCRIPTION: &str = "Use Airbnb only for explicit Airbnb or travel lodging/accommodation/booking intent: stays, dates, guests, budget for stays, home/apartment rental, vacation rental, or short-term stay search. Do not use for city facts, local weather, geography, maps, routes, or generic destination discussion without lodging intent.";
...
pub fn brain_maps_codeact_template() -> BrainGeneralCodeActTemplate {
    let mut template = BrainGeneralCodeActTemplate {
        ...
        section: "webexplorer".to_string(),
        purpose: "Open the contained native WebExplorer on Google Earth while the assistant keeps conversing in the left transcript. Use this for cities, places, countries, regions, local weather at a place, where-is questions, maps, routes, coordinates, Google Earth, or ordinary geographic context. Prefer it over /sciencebrain_, /airbnb_, and /googleweb_ unless the user clearly asks for scientific modeling or lodging/accommodation. Default uses the Brain home city when no target is specified; optional latitude and longitude slots let the LLM target explicit WGS84 coordinates. Device location must never be read silently; current-location flows require explicit user permission before any coordinate is used.".to_string(),
...
pub fn brain_airbnb_codeact_template() -> BrainGeneralCodeActTemplate {
    let mut template = BrainGeneralCodeActTemplate {
        ...
        section: "webexplorer".to_string(),
        purpose: "Open the contained split-screen WebExplorer on Airbnb.com. Use this command only when the user mentions Airbnb or clearly asks for lodging/accommodation/booking: trip stays, house/apartment/home rental, destination dates for a stay, guests, budget for stays, vacation rentals, or short-term stay search. Do not use it for city facts, local weather, geography, maps, routes, or generic destination discussion without lodging intent; those belong to /maps_. The LLM must write its own natural user-facing sentence adapted to the user's request before activating the command, and must also copy that same LLM-authored sentence into the say slot so command-only render paths still show a normal answer. The host must not synthesize that sentence. The LLM then activates this CodeAct, and the application renders the action event automatically.".to_string(),
