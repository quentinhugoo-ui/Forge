...
const GOOGLE_OAUTH_START_PATH: &str = "/oauth/google/start";
const GOOGLE_OAUTH_CALLBACK_PATH: &str = "/oauth/google/callback";
const GMAIL_ACCESS_TOKEN_PATH: &str = "/api/gmail/access-token";
const GMAIL_OAUTH_SCOPES: [&str; 3] = [
...
fn handle_connection(mut stream: TcpStream) -> Result<(), String> {
    ...
    if let Some(target) = request_target(&request_line) {
        let (path, query) = request_path_and_query(target);
        if request_line.starts_with("GET ") && path == GOOGLE_OAUTH_START_PATH {
            return write_google_oauth_start(&mut stream);
        }
        if request_line.starts_with("GET ") && path == GOOGLE_OAUTH_CALLBACK_PATH {
            return write_google_oauth_callback(&mut stream, query);
        }
        if request_line.starts_with("OPTIONS ") && path == GMAIL_ACCESS_TOKEN_PATH {
            return write_empty_response(&mut stream, 204);
        }
        if request_line.starts_with("GET ") && path == GMAIL_ACCESS_TOKEN_PATH {
            return write_gmail_access_token(&mut stream, &auth_header);
        }
...
fn write_google_oauth_start(stream: &mut TcpStream) -> Result<(), String> {
    match google_oauth_authorize_url() {
        Ok(url) => write_redirect_response(stream, &url),
        Err(error) => write_html_response(
            stream,
            503,
            &format!(
                "<h1>Gmail OAuth configuration incomplete</h1><p>{}</p>",
                escape_html(&error)
            ),
        ),
    }
...
fn write_google_oauth_callback(stream: &mut TcpStream, query: &str) -> Result<(), String> {
    if let Some(error) = query_param(query, "error") {
        ...
        return write_html_response(
            ...
            400,
            &format!(
                "<h1>Gmail OAuth refuse</h1><p>{}</p><p>{}</p>",
                escape_html(&error),
                escape_html(&description)
            ),
        );
    }
    if let Some(expected_state) = google_oauth_state() {
        let presented_state = query_param(query, "state").unwrap_or_default();
        if presented_state != expected_state {
            return write_html_response(
                stream,
                401,
                "<h1>Gmail OAuth bloque</h1><p>Le parametre state ne correspond pas.</p>",
            );
        }
    }
    let Some(code) = query_param(query, "code") else {
        return write_html_response(
            stream,
            400,
            "<h1>Gmail OAuth incomplet</h1><p>Google n'a pas renvoye de code OAuth.</p>",
        );
    };
    match exchange_google_oauth_code(&code) {
        Ok(token) => {
            let refresh = token.refresh_token.unwrap_or_default();
            let refresh_block = if refresh.is_empty() {
                "<p>Google n'a pas renvoye de refresh token. Relance /oauth/google/start avec prompt=consent ou retire l'acces de l'app dans ton compte Google, puis recommence.</p>".to_string()
            } else {
                format!(
                    "<p>Ajoute cette variable dans Render puis redemarre le service :</p><pre>GOOGLE_OAUTH_REFRESH_TOKEN={}</pre>",
                    escape_html(&refresh)
                )
            };
            write_html_response(
                ...
                200,
                &format!(
                    "<h1>Gmail OAuth connecte</h1><p>Le bridge Gmail peut maintenant generer des access tokens courts sans exposer le refresh token au LLM.</p>{}<p>Garde cette page privee et ferme-la apres copie.</p>",
                    refresh_block
                ),
            )
        }
...
fn write_gmail_access_token(stream: &mut TcpStream, auth_header: &str) -> Result<(), String> {
    let Some(secret) = gmail_token_proxy_secret() else {
        return write_json_response(
            stream,
            503,
            &json!({
                "schema": "forge.gmail.oauth_access_token.v1",
                "status": "blocked",
                "error": "gmail_token_proxy_secret_missing"
            }),
        );
    };
    if !bearer_matches(auth_header, &secret) {
        return write_json_response(
            stream,
            401,
            &json!({
    ...
    match refresh_google_oauth_access_token() {
        Ok(token) => {
            let Some(access_token) = token.access_token else {
                return write_json_response(
                    ...
                    &json!({
                        "schema": "forge.gmail.oauth_access_token.v1",
                        "status": "error",
                        "error": "google_token_response_missing_access_token"
                    }),
                );
            };
            write_json_response(
                ...
                &json!({
                    ...
                    "expiresIn": token.expires_in.unwrap_or(0),
                    "scope": token.scope.unwrap_or_default(),
                    "tokenType": token.token_type.unwrap_or_else(|| "Bearer".to_string())
                }),
            )
        }
...
fn google_oauth_authorize_url() -> Result<String, String> {
    let client_id = google_oauth_client_id().ok_or_else(|| "GOOGLE_OAUTH_CLIENT_ID is missing".to_string())?;
    ...
    if let Some(state) = google_oauth_state() {
...
fn exchange_google_oauth_code(code: &str) -> Result<GoogleOAuthTokenResponse, String> {
    let client_id = google_oauth_client_id().ok_or_else(|| "GOOGLE_OAUTH_CLIENT_ID is missing".to_string())?;
    let client_secret = google_oauth_client_secret().ok_or_else(|| "GOOGLE_OAUTH_CLIENT_SECRET is missing".to_string())?;
    let redirect_uri = google_oauth_redirect_uri();
    google_oauth_token_request(vec![
        ("client_id", client_id),
...
fn refresh_google_oauth_access_token() -> Result<GoogleOAuthTokenResponse, String> {
    let client_id = google_oauth_client_id().ok_or_else(|| "GOOGLE_OAUTH_CLIENT_ID is missing".to_string())?;
    let client_secret = google_oauth_client_secret().ok_or_else(|| "GOOGLE_OAUTH_CLIENT_SECRET is missing".to_string())?;
    let refresh_token = google_oauth_refresh_token().ok_or_else(|| "GOOGLE_OAUTH_REFRESH_TOKEN is missing".to_string())?;
    google_oauth_token_request(vec![
        ("client_id", client_id),
        ("client_secret", client_secret),
...
fn google_oauth_token_request(form: Vec<(&'static str, String)>) -> Result<GoogleOAuthTokenResponse, String> {
    let client = reqwest::blocking::Client::builder()
        .timeout(Duration::from_secs(15))
        .build()
        .map_err(|err| format!("google oauth client: {err}"))?;
    let body = form
        .iter()
        .map(|(key, value)| format!("{}={}", key, url_encode(value)))
        .collect::<Vec<_>>()
    ...
    let response = client
        ...
        .body(body)
        .send()
        .map_err(|err| format!("google oauth request: {err}"))?;
    let status = response.status().as_u16();
    let token: GoogleOAuthTokenResponse = response
        .json()
        .map_err(|err| format!("google oauth parse: {err}"))?;
    if status >= 400 || token.error.is_some() {
        return Err(format!(
            "google oauth status {status}: {} {}",
            token.error.unwrap_or_else(|| "token_error".to_string()),
...
fn write_redirect_response(stream: &mut TcpStream, location: &str) -> Result<(), String> {
    let response = format!(
        "HTTP/1.1 302 Found\r\nLocation: {location}\r\nContent-Length: 0\r\nConnection: close\r\n\r\n"
    );
    stream
        .write_all(response.as_bytes())
        .map_err(|err| format!("write redirect: {err}"))?;
    stream.flush().map_err(|err| format!("flush redirect: {err}"))
}
...
fn write_html_response(stream: &mut TcpStream, status: u16, body: &str) -> Result<(), String> {
    ...
    let page = format!(
        "<!doctype html><html><head><meta charset=\"utf-8\"><title>Forge Gmail OAuth</title><style>body{{font:16px system-ui;background:#111;color:#eee;max-width:760px;margin:48px auto;padding:0 24px}}pre{{white-space:pre-wrap;background:#1f1f1f;border:1px solid #333;border-radius:8px;padding:16px;color:#55d6ff}}</style></head><body>{body}</body></html>"
    );
    let response = format!(
    ...
    stream
        .write_all(response.as_bytes())
        .map_err(|err| format!("write html response: {err}"))?;
    stream.flush().map_err(|err| format!("flush html response: {err}"))
}

fn google_oauth_client_id() -> Option<String> {
    env_first(&["GOOGLE_OAUTH_CLIENT_ID", "GOOGLE_CLIENT_ID", "GMAIL_OAUTH_CLIENT_ID"])
}
...
fn google_oauth_refresh_token() -> Option<String> {
    env_first(&[
...
fn google_oauth_redirect_uri() -> String {
    env_first(&["GOOGLE_OAUTH_REDIRECT_URI", "GMAIL_OAUTH_REDIRECT_URI"]).unwrap_or_else(|| {
        std::env::var("RENDER_EXTERNAL_URL")
            .ok()
            .map(|value| value.trim().trim_end_matches('/').to_string())
            .filter(|value| !value.is_empty())
            .map(|base| format!("{base}{GOOGLE_OAUTH_CALLBACK_PATH}"))
            .unwrap_or_else(|| "https://forge-6cai.onrender.com/oauth/google/callback".to_string())
    })
}

fn google_oauth_state() -> Option<String> {
    env_first(&["GOOGLE_OAUTH_STATE", "GMAIL_OAUTH_STATE"])
}

fn gmail_token_proxy_secret() -> Option<String> {
    env_first(&[
        "FORGE_GMAIL_TOKEN_PROXY_SECRET",
        "GMAIL_TOKEN_PROXY_SECRET",
        "FORGE_TOKEN",
    ])
}
...
fn bearer_matches(auth_header: &str, expected: &str) -> bool {
    auth_header
        .strip_prefix("Bearer ")
...
fn query_param(query: &str, key: &str) -> Option<String> {
    query.split('&').find_map(|part| {
        ...
        if form_url_decode(raw_key) == key {
            Some(form_url_decode(raw_value))
        } else {
            None
        }
    })
