...
function brainBootManifest(): string {
  ...
  return [
    ...
    `scrapers_mcp_policy=${BRAIN_SCRAPERS_COMMAND} is the preferred Brain route when web collection should combine robust selector extraction with clean RAG-ready Markdown. Activate it for one or more target URLs when the result should feed Brain memory, RAG, agents, research notes, comparisons, or downstream structured analysis. The host/agent must fan out to Scrapling MCP and Crawl4AI MCP at the same time, wait for bounded results or record partial failures, then return one compact merged manifest with structured fields, fit Markdown or chunks, citations, fetch metadata and provenance. Respect robots.txt, site terms, rate limits and privacy; use stealth or anti-bot modes only for authorized targets or user-approved public-data collection.`,
