...
function webExplorerCodeActInstructions(moduleId = ""): string {
  if (moduleId === "gmail") {
    return [
      ...
      `Use intent="search" or intent="summarize" with mode="gmail_api" when the user asks to check, read, search, inspect, report, summarize, find new/unread/incoming mail, or asks s'il y a du nouveau, even if the wording says open/ouvre la boite mail.`,
      `Use intent="open" mode="split_webexplorer" only when the user wants visual Gmail navigation without mailbox analysis.`,
...
function brainSegmentManifest(): string {
  ...
  return [
    ...
    "active=general",
    brainPersonalityContextManifest(),
    `mandatory=Before answering or acting, classify the user's task by semantic domain. If key choices, constraints, scope, files, risks or success criteria are missing, you may use ${BRAIN_QUESTIONNAIRE_COMMAND} from General Brain before acting. If the task is already clear and belongs to science, engineering, mathematics, biology, chemistry, physics, cryptography, formal analysis, electronics, mechanics, robotics, CAD/3D engineering, physical product/prototype engineering, Banger, future Banger 3D, Monster or /newcompute_ work, write a short natural acknowledgement and activate ${BRAIN_SCIENCE_COMMAND} before detailed specialized work.`,
    "mandatory=Do not activate /sciencebrain_ only because the user asks to look at images of physical objects, artworks, sculptures, ceramics, forms, style, composition or aesthetics; keep that in General Brain unless the user explicitly asks for material science, structure, manufacturing, CAD, simulation, measurements or engineering constraints.",
