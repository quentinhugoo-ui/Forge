...
describe("Coding Brain live preview CodeAct", () => {
  it("registers a Coding-only visual preview command after real file writes", () => {
    ...
    expect(brainSource).toContain('pub const BRAIN_LOCAL_ACTIONS_COMMAND: &str = "/local_actions_";');
    expect(brainSource).not.toContain('pub const BRAIN_CAPABILITIES_COMMAND: &str = "/capabilities_";');
    expect(brainSource).not.toContain("brain_capabilities_codeact_template()");
    expect(brainSource).toContain("Use /local_actions_ when coding work may need local files");
    expect(brainSource).toContain("Use /local_actions_ when scientific/engineering work may require local files");
    expect(brainSource).toContain("brain_local_actions_codeact_template()");
    ...
    expect(generatedSource).toContain("BRAIN_LOCAL_ACTIONS_COMMAND");
    expect(generatedSource).toContain("BRAIN_CODING_LIVE_PREVIEW_COMMAND_DESCRIPTION");
    expect(generatedSource).toContain("BRAIN_LOCAL_ACTIONS_COMMAND_DESCRIPTION");
    expect(generatedSource).not.toContain("BRAIN_CAPABILITIES_COMMAND");
    expect(ipcSource).toContain("BRAIN_CODING_LIVE_PREVIEW_COMMAND");
    expect(ipcSource).toContain("BRAIN_LOCAL_ACTIONS_COMMAND");
    expect(mainSource).toContain("visual_artifact_rule=");
    expect(mainSource).toContain("AGENT_ACTION_JSON first");
    expect(mainSource).toContain("Coding Brain keeps Windows/local action tools");
    expect(mainSource).toContain("do not write a copy-paste code block");
  ...
  it("forces visual coding snippets back into the Windows tool loop", () => {
    ...
    expect(mainSource).toContain("creates or modifies the local file");
    expect(mainSource).toContain("Do not claim file proof before AGENT_ACTION_RESULT");
    ...
    expect(mainSource).toContain("The local visual file exists, but the live preview has not been opened yet");
    expect(mainSource).toContain("emit ${BRAIN_CODING_LIVE_PREVIEW_COMMAND}");
