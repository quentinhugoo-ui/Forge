...
describe("Banger Cesium viewport contract", () => {
  it("keeps the Banger page as a full-screen render surface with no inner frame prose", () => {
    ...
    expect(routerSource).toContain('className={cesiumLive ? "bangerCesiumViewport bangerCesiumViewport--live" : "bangerCesiumViewport"}');
  ...
  it("keeps Cesium fail-visible instead of returning a black silent canvas", () => {
    ...
    expect(routerSource).toContain("scene.renderError.addEventListener");
    expect(routerSource).toContain("camera.computeViewRectangle(Cesium.Ellipsoid.WGS84)");
    ...
    expect(stylesSource).toContain(".bangerCesiumViewport--live::before");
