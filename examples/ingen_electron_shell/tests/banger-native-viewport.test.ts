...
describe("Banger native viewport contract", () => {
  it("keeps the Banger page as a full-screen render surface with no inner frame prose", () => {
    ...
    expect(routerSource).toContain("cesiumjs_google_earth_style_globe_with_photorealistic_tiles_overlay");
    ...
    expect(routerSource).toContain("BANGER_GLOBE_ORBIT_HEIGHT_METERS");
    expect(routerSource).toContain("viewer.resize()");
    expect(routerSource).toContain("Cesium.Math.toRadians(-90)");
    expect(routerSource).not.toContain("Cesium.Math.toRadians(-62)");
  ...
  it("launches the Rust native child-surface host instead of only probing offscreen", () => {
    ...
    expect(nativeBridgeSource).toContain("Depth32Float");
    expect(nativeBridgeSource).toContain("draw_indexed");
    expect(nativeBridgeSource).toContain("draw_indexed_indirect");
    expect(nativeBridgeSource).toContain("meshlet_culled_indirect_draw_buffer");
    ...
    expect(nativeBridgeSource).toContain("visible_tile_batch_indexed_mesh_wgpu_draw_ready");
    ...
    expect(mainSource).not.toContain("google-map-tiles-direct");
    expect(mainSource).not.toContain("google-map-tiles-api-key");
    expect(mainSource).toContain("none_direct_cesium_required");
    expect(mainSource).toContain("cesium_ion_credentials_required");
  ...
  it("paints Maps from the Banger native sphere host and forbids CesiumJS/WebGL regressions", () => {
    ...
    expect(mainSource).toContain("directRootTilesetEndpoint: \"ion://google-photorealistic-3d-tiles\"");
    expect(mainSource).toContain("accessMode: \"cesium-ion\"");
    expect(mainSource).not.toContain("accessMode: \"google-map-tiles-api-key\"");
