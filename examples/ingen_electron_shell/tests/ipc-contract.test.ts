...
describe("typed header IPC contract", () => {
  ...
  it("accepts bounded agent action host requests", () => {
    ...
    expect(isAgentActionRequest({ action: "document_toolchain_inspect", query: "all" })).toBe(true);
    expect(isAgentActionRequest({ action: "document_toolchain_install", query: "media", confirmed: true })).toBe(true);
    ...
    expect(isAgentActionRequest({ action: "dev_github_pr_review_submit", query: "1", command: "comment", content: "Review", confirmed: true })).toBe(true);
    ...
    expect(isAgentActionRequest({ action: "windows_sensitive_inspect", settingName: "firewall" })).toBe(true);
    expect(isAgentActionRequest({ action: "windows_sensitive_apply", settingName: "user_env", query: "INGEN_TEST", content: "1", confirmed: true })).toBe(true);
    ...
    expect(isAgentActionRequest({ action: "ci_rerun_failed", query: "123", confirmed: true })).toBe(true);
