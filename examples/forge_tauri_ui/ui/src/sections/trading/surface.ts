...
(function () {
  ...
  function hasTauriInvoke() {
    return typeof window !== "undefined" && !!(window.ForgeShellRuntime?.tauri?.invoke || window.ForgeTauriBridge?.invoke);
  }
  ...
  async function invoke(command, args = {}, options = {}) {
    ...
    if (runtimeInvoke) return runtimeInvoke(command, args, { section: "trading", ...options });
    return Promise.reject(new Error(`Forge runtime bus unavailable: ${command}`));
  }
  ...
  function normalizeTradingChatSubbarMode(mode = "none") {
    return window.ForgeTradingState?.normalizeChatSubbarMode?.(mode) || "none";
  }
  ...
  function normalizeTradingSubbarSection(section = "indicators") {
    return window.ForgeTradingState?.normalizeSubbarSection?.(section) || "indicators";
  }
  ...
  function renderOrdersPanel(container) {
    ...
    const refresh = () => {
      ...
      const currentSide = state.orderForm.side || "BUY";
      const brokerUavLong = Number((price as any)?.unitsAvailableLong) || 0;
      const brokerUavShort = Number((price as any)?.unitsAvailableShort) || 0;
      const brokerUav = currentSide === "BUY" ? brokerUavLong : brokerUavShort;
      const uAv = brokerUav > 0 ? brokerUav : unitsAvailableApprox(instrument, ask, currency, marginAvailable);
  ...
  function deactivateTrading() {
    if (!state.active) return;
    state.active = false;
    tradingController?.publishActive?.(false);
    Object.assign(state, window.ForgeTradingState?.deactivatePatch?.() || {
      chatSubbarMode: "none",
      chatSubbarExpanded: false,
      chatSubbarSection: "indicators",
      indicatorsModeActive: false,
  ...
  tradingController = window.ForgeTradingController?.create?.({
    runtime: window.ForgeShellRuntime,
    button: els.button,
    isActive: () => !!state.active,
    isBlocked: () => !!window.__forgeRealEstateModeActive,
    isWebExplorerActive: () => !!window.__forgeWebExplorerIsActive?.(),
    activate: activateTrading,
    deactivate: deactivateTrading,
    focus: focusTradingSurface,
    trace: (stage, details) => {
      try { window.alphaTrace?.(stage, details || {}); } catch (_) {}
    },
  }) || null;
